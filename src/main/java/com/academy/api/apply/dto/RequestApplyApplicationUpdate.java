...
@Setter
@NoArgsConstructor
@Schema(description = "원서접수 수정 요청")
public class RequestApplyApplicationUpdate {
    ...
    @Pattern(regexp = "^\\d{3}-\\d{4}-\\d{4}$", message = "휴대폰 형식이 올바르지 않습니다")
    ...
    @Schema(description = "신청 과목 목록", example = "[\"KOR\", \"ENG\", \"MATH\"]")
    ...
    @Schema(description = "수정 사유 (이력에 기록됨)", example = "연락처 정보 변경", 
            requiredMode = Schema.RequiredMode.REQUIRED)
